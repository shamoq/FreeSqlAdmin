...
namespace Simple.Utils.Extensions
{
    ...
    public static class TypeExtension
    {
        ...
        /// <returns></returns>
        /// <![CDATA[
        /// 获得比如被Task，ICollection<>，IEnumable<>,IQueryable<>等包裹的类型
        /// ]]>
        ...
        {
            if (string.IsNullOrEmpty(typeName)) throw new Exception("typeName must be not empty");
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException"></exception>
        public static TResult GetPropertyValue<T, TResult>(T model, string propertyName)
        {
            var result = GetPropertyValue(model, propertyName);
            return (TResult)result;
        }

        public static TResult GetPropertyValueByEmit<T, TResult>(T model, string propertyName)
        {
        ...
        public static object GetPropertyValueByEmit<T>(T model, string propertyName)
        {
            var type = model.GetType();
            var property = type.GetProperty(propertyName);
        ...
        public static DataTable ToDataTable<T>(this IEnumerable<T> source, List<PropertyInfo> propertyInfos = null, bool useColumnAttribute = false) where T : class
        {
            ...
            foreach (var propertyInfo in propertyInfos)
            {
                ...
                table.Columns.Add(columnName, GetNotNullType(propertyInfo.PropertyType));
            }

            Func<T, object[]> func;
            var key = string.Join(",", typeof(T).FullName + propertyInfos.Select(it => it.Name).ToList());
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static Type GetNotNullType(Type type)
        {
            if (type.IsNullable())
            {
