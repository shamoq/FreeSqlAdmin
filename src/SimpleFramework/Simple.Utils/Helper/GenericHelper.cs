...
{
    /// <summary>泛型扩展</summary>
    public static class GenericHelper
    {
        #region ToDictionary

        public static Dictionary<string, string> ToDictionary<T>(T t, Dictionary<string, string> dic = null) where T : class
        {
            if (dic == null)
        ...
        public static Dictionary<string, string> ToDictionary<TInterface, T>(TInterface t, Dictionary<string, string> dic = null) where T : class, TInterface
        {
            if (dic == null)
                dic = new Dictionary<string, string>();
        ...
        public static T ParseTo<T>(string str) where T : struct
        {
            return ParseTo(str, default(T));
        }
        ...
        /// <returns>返回指定的类型。</returns>
        public static T ParseTo<T>(string str, T defaultValue) where T : struct
        {
            var t = ParseToNullable<T>(str);
            if (t.HasValue)
            {
                return t.Value;
            }
        ...
        /// <param name="defaultValue">如果转换失败，需要使用的默认值</param>
        /// <returns>返回指定的类型。</returns>
        public static T ParseToAny<T>(string str, T defaultValue = default)
        {
        ...
        /// <param name="str">需要转换的字符串</param>
        /// <returns>返回指定的类型</returns>
        public static T? ParseToNullable<T>(string str) where T : struct
        {
            if (string.IsNullOrEmpty(str))
            {
                return null;
        ...
        /// <returns></returns>
        public static TOut FastClone<TIn, TOut>(TIn source)
        {
            return ObjectFastClone<TIn, TOut>.Clone(source);
        }

        /// <summary>对IP地址列表实现排序</summary>
        /// <param name="ips"></param>
        /// <param name="asc">true为升序，false为降序</param>
        /// <returns></returns>
        public static ICollection<string> Order(ICollection<string> ips, bool asc = true)
        {
            if (ips == null)
        ...
        public static void SaveToCsv<T>(IEnumerable<T> source, string csvFullName, string separator = ",")
        {
            if (source == null)
    ...
    /// <summary>运用表达式树实现对象的快速复制</summary>
